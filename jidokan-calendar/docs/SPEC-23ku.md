...
3. 座標と color: **`ingest/geocode.py` に任せる**（手作業でNominatimを叩かない）。
   `lat`/`lng` はキーごと省略した状態で JSON を作り、下記を実行すれば付与される。
4. 検証（**全件必須**）: **`ingest/verify_ward.py` に任せる**（手作業でcurlを回さない）。
5. `wards.json` の該当区を `covered` に変更。
6. 報告: 館数、除外した施設と理由（例: 中高生専用）、`verify_ward.py` の最終結果。

#### 8.1 ツール（手順3・4はこの2本で完結する）

```
python3 ingest/geocode.py --ward <wardId> --fill-color   # 座標付与(1req/秒厳守)＋§4.4のcolor巡回付与
python3 ingest/verify_ward.py --ward <wardId>            # 静的＋ネット検証を全件、PASS/FAILで終了
```

- `geocode.py`: 住所を「〒除去→番地除去→丁目まで」と段階的に短縮して再試行し、
  東京23区のbbox外に落ちた結果は棄却する。`--all` で全館引き直し、`--dry-run` で書き込まない。
  失敗した館は住所を見直して再実行する。
- `verify_ward.py`: 静的（JSON valid・必須フィールド・id形式/重複・color巡回・pdfUrl重複・
  **座標の外れ値**＝区内他館の中央値から6km超）と、ネット（`officialUrl`/`pdfUrl` の HTTP 200・
  全館 `fetch_source()`）をまとめて実行する。`--offline` で静的のみ（高速）。
  **FAIL が消えるまで直して再実行する**のがこの手順の完了条件。
- `pdfUrl` に一覧ページを置いた館は「一覧ページ→PDF の自動解決」として解決先URLが表示される。
  これは §8手順2 どおりの正常な挙動だが、**解決先のファイル名が当月か・中高生向けおたよりを
  拾っていないかは人／モデルが目視で確認する**（機械判定は不可）。旧月や中高生向けを拾う館だけ
  `pdfUrl` を当月PDFの直リンクに差し替え、月替わりで要修正である旨を報告に残す。

これらは決定的な処理のみを担うので精度は落ちない。**判断が要る手順1・2（一覧の収集、
pdfUrl の選定、§8.3の除外判定）はモデルの担当**であり、ツールでは代替できない。
...
- **HTTP 200 かつ fetch_source 成功でも、中身が予定表本文でなければ除外**。`verify_ward.py` の
  「中身が予定表でない疑い」警告が該当する（日付表記が3件未満のテキスト）。
  判例: **品川区**は区全体を未対応とした（2026-07 判定）。児童センター25館の予定表が
  ポータル `shinagawa-pocket.city-hc.jp` のJS描画スキャン画像で配信されており、
  `fetch_source` では「〇〇児童センターお知らせ 7月号」というタイトル列（約400字・日付0件）
  しか取れず、取り込んでもイベントが1件も得られないため。covered にすると「実データ」表示
  なのに常に空、という状態になる。
  なお同区の「IKUMOやしお」1館のみPDFおたよりが公開されており警告対象外だが、
  1館だけでは区として成立しないため保留。`ingest.py` が `<img>` を追えるよう拡張されれば
  区ごと再開できる（スキャン画像自体は Gemini が直接読める）。
