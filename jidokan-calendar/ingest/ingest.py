...
def encode_url(url):
    """非ASCIIを含むURLを percent-encode する。

    児童館サイトは「7月号　最新.pdf」のような日本語ファイル名のPDFを貼ることが多く、
    そのまま urlopen に渡すと UnicodeEncodeError で落ちる。既に percent-encode 済みの
    URLを二重変換しないよう '%' は safe に含める。"""
