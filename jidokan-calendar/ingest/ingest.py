...
- カレンダー形式（日付のマス目・表）の場合は、各マス目（日付欄）を1つずつ確認し、
  書かれている行事を必ず全て拾う。各イベントを正しい日付の列／行に対応づけ、
  ひとつ隣の日付に取り違えないよう罫線・曜日の並びをよく確認する。
...
def gemini_extract(mime, data, api_key, year, log=lambda *a: None):
    ...
    body = json.dumps({
        ...
        "generationConfig": {
            ...
            "temperature": 0,
            # 出力の途中切れ(JSON破損)を防ぐため、思考(thinking)を無効化し出力枠を広く取る。
            # 2.5系は thinking が出力トークンを消費するため、複雑なカレンダーでは特に効く。
    ...
    for c in centers:
        ...
        try:
            ...
            if used != url:
                ...
                log(f"    形式: {kind}")
            events = gemini_extract(mime, data, api_key, year, log=log)
