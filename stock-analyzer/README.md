...
**https://jyunjh.github.io/claude-projects/stock-analyzer/**

`main` に push すると GitHub Actions が自動デプロイします。スマホのブラウザからそのまま開けます。

> 公開版は**閲覧のみ**です。スナップショット保存はサーバ側の書き込みが必要なため、
> 手元で `serve.py` を動かしたときだけ有効になります（ボタン自体が出ません）。
> APIキーは端末ごとのブラウザ(localStorage)に保存されるので、
> 公開版で「🔄 最新に更新」を使うにはその端末でキーを入れてください。
